...
func GetWorld(name string) *World {
	...
	loadedWorld, err := LoadWorld(name)
	if err != nil {
		// If the world doesn't exist, create a new one and save it
		return GenerateWorld(name, 100, 60)
	}
...
func SaveWorld(world *World) error {
	if world == nil {
		return errors.New("SaveWorld: world is nil")
	}
	filePath := "database/worlds/_" + world.Name + ".bin"
	...
	if err != nil {
		log.Fatal("Encoding failed: %v", err)
		return err
	}
...
func LoadWorld(name string) (*World, error) {
	filePath := "database/worlds/_" + name + ".bin"
