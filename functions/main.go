...
func SendWorldMenu(peer enet.Peer) {
	...
	for _, world := range worlds.Worlds {
		world_packet += "add_floater|" + world.Name + "|" + world.Name + "|" + strconv.Itoa(int(world.TotalPlayer)) + "|0.5|3529161471\n"
	}
...
func OnConsoleMessage(peer enet.Peer, message string, delay int) {
	variant := variant.NewVariant(delay, -1)
	variant.InsertString("OnConsoleMessage")
	variant.InsertString(message)
	variant.Send(peer)
}
...
func OnTalkBubble(peer enet.Peer, netID int, text string, isOverlay bool) {
	variant := variant.NewVariant(0, -1)
	variant.InsertString("OnTalkBubble")
	variant.InsertInt(netID)
...
func OnSuperMain(peer enet.Peer, itemHash uint32) {
	...
	variant.InsertString("0098/2604202401/cache/")
